...
describe("NotificationsService", () => {
  ...
  let notificationDispatchService: { sendEmail: jest.Mock; sendPush: jest.Mock };
  let pushTokensService: { findAllForUser: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    notificationDispatchService = { sendEmail: jest.fn(), sendPush: jest.fn() };
    pushTokensService = { findAllForUser: jest.fn().mockResolvedValue([]) };
  ...
  it("sends a push to every one of the customer's registered devices", async () => {
    pushTokensService.findAllForUser.mockResolvedValue([
    ...
    expect(notificationDispatchService.sendPush).toHaveBeenCalledTimes(2);
    expect(notificationDispatchService.sendPush).toHaveBeenCalledWith(
      expect.objectContaining({ to: "ExponentPushToken[a]" }),
      { relatedType: "ORDER_UPDATE", relatedId: "o1" },
    );
  });

  it("is a silent no-op for push when the customer has no registered devices", async () => {
    await service.handleOrderStatusChanged(new OrderStatusChangedEvent("o1", "PLACED" as never, "CONFIRMED" as never, "staff1"));

    expect(notificationDispatchService.sendPush).not.toHaveBeenCalled();
  });

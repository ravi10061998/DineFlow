...
Single Next.js app, role-scoped route groups — not separate apps per portal. `/admin/*` (ADMIN), `/restaurant/*` (RESTAURANT_ADMIN/STAFF), `/` is the full customer storefront home page (Module 16) — an ADMIN/RESTAURANT_* user landing there sees a "Dashboard" link in the header rather than being force-redirected, `/login` + `/register` + `/register-restaurant` public.
